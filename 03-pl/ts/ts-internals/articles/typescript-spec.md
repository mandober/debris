# TS Language Specification

Version 1.8 (Jan 2016)
...
TS helps programming teams to define interfaces between software components and to gain insight into the behavior of existing JS libraries. TS also enables teams to reduce naming conflicts by organizing their code into dynamically-loadable modules. TS's optional type system enables JS programmers to use highly-productive development tools and practices: static checking, symbol-based navigation, statement completion, and code refactoring.

TS is a syntactic sugar for JS. 

TS syntax is a superset of ECMAScript 2015 (ES2015) syntax. 

>Every JS program is also a TS program.

>TS compiler performs only file-local transformations on TS programs and does not re-order variables declared in TS.

This leads to JS output that closely matches the TS input. 

TS does not transform variable names, making tractable the direct debugging of emitted JS. TS optionally provides source maps, enabling source-level debugging. TS tools typically emit JS upon file save, preserving the test, edit, refresh cycle commonly used in JS development.

TS syntax includes all features of ECMAScript 2015, including classes and modules, and provides the ability to translate these features into ECMAScript 3 or 5 compliant code.

Classes enable programmers to express common object-oriented patterns in a standard way, making features like inheritance more readable and interoperable. Modules enable programmers to organize their code into components while avoiding naming conflicts. The TS compiler provides module code generation options that support either static or dynamic loading of module contents.

TS also provides to JS programmers a system of optional type annotations. These type annotations are like the JSDoc comments found in the Closure system, but in TS they are integrated directly into the language syntax. This integration makes the code more readable and reduces the maintenance cost of synchronizing type annotations with their corresponding variables.

The TS type system enables programmers to express limits on the capabilities of JS objects, and to use tools that enforce these limits. To minimize the number of annotations needed for tools to become useful, the TS type system makes extensive use of type inference. For example, from the following statement, TS will infer that the variable 'i' has the type number.

```TS
var i = 0;
```

TS will infer from the following function definition that the function f has return type string.
...
To benefit from this inference, a programmer can use the TS language service. For example, a code editor can incorporate the TS language service and use the service to find the members of a string object as in the following screenshot.

&emsp;&emsp;![](images/image1.png)

In this example, the programmer benefits from type inference without providing type annotations. Some beneficial tools, however, do require the programmer to provide type annotations. In TS, we can express a parameter requirement as in the following code fragment.
...
This optional type annotation on the parameter 's' lets the TS type checker know that the programmer expects parameter 's' to be of type 'string'. Within the body of function 'f', tools can assume 's' is of type 'string' and provide operator type checking and member completion consistent with this assumption. Tools can also signal an error on the first call to 'f', because 'f' expects a string, not an object, as its parameter. For the function 'f', TS will emit the following JS code:
...
In the JS output, all type annotations have been erased. In general, TS erases all type information before emitting JS.

## 1.1 Ambient Declarations

>An *ambient declaration* introduces a variable into a TS scope, but has zero impact on the emitted JS code.

We use ambient declarations to tell TS that some other component will supply a variable.

By default, TS prints an error when referencing an undefined variable. We can add ambient declarations to "register" some common browser-defined variables.

The following example declares the `document` object. Since the declaration doesn't specify a type, `any` is inferred.

>`any` type means we can assume nothing about the shape or behavior of the type.

```ts
declare var document

// Ok, because document was declared
document.title = "Hello"
```

Some of the examples below will illustrate how programmers can use types to further characterize expected behaviors of objects.

In the case of `document`, TS automatically supplies a declaration, because TS by default includes a file `lib.d.ts` that provides *interface declarations* for the built-in JS library as well as for the DOM.

TS does not include by default an interface for jQuery, so to use jQuery, we may supply a declaration such as:
...
Function expressions are a powerful feature of JS. They enable function definitions to create closures: functions that capture information from the lexical scope surrounding the function's definition. Closures are currently JS's only way of enforcing data encapsulation. By capturing and using environment variables, a closure can retain information that cannot be accessed from outside the closure. JS programmers often use closures to express event handlers and other asynchronous callbacks, in which another software component, such as the DOM, will call back into JS through a handler function.

TS function types make it possible for programmers to express the expected *signature* of a function. A function signature is a sequence of parameter types plus a return type. The following example uses function types to express the callback signature requirements of an asynchronous voting mechanism.
...
TS programmers use *object types* to declare their expectations of object behavior. The following code uses an *object type literal* to specify the return type of the 'MakePoint' function.
...
TS object types model the diversity of behaviors that a JS object can exhibit. For example, the jQuery library defines an object, '$', that has methods, such as 'get' (which sends an Ajax message), and fields, such as 'browser' (which gives browser vendor information). However, jQuery clients can also call '$' as a function. The behavior of this function depends on the type of parameters passed to the function.
...
We mentioned above that the '$' function behaves differently depending on the type of its parameter. So far, our jQuery typing only captures one of these behaviors: return an object of type 'JQuery' when passed a string. To specify multiple behaviors, TS supports *overloading* of function signatures in object types. For example, we can add an additional call signature to the 'JQueryStatic' interface.

```TS
(ready: () => any): any;
```

This signature denotes that a function may be passed as the parameter of the '$' function. When a function is passed to '$', the jQuery library will invoke that function when a DOM document is ready. Because TS supports overloading, tools can use TS to show all available function signatures with their documentation tips and to give the correct documentation once a function has been called with a particular signature.
...
Ordinarily, TS type inference proceeds "bottom-up": from the leaves of an expression tree to its root. In the following example, TS infers 'number' as the return type of the function 'mul' by flowing type information bottom up in the return expression.
...
For variables and parameters without a type annotation or a default value, TS infers type 'any', ensuring that compilers do not need non-local information about a function's call sites to infer the function's return type. Generally, this bottom-up approach provides programmers with a clear intuition about the flow of type information.
...
$.get("http://mysite.org/divContent",  
      function (data) {  
          $("div").text(data);  // TS infers data is a string  
      }  
);
```
...
JS practice has two very common design patterns: the module pattern and the class pattern. Roughly speaking, the module pattern uses closures to hide names and to encapsulate private data, while the class pattern uses prototype chains to implement many variations on object-oriented inheritance mechanisms. Libraries such as 'prototype.js' are typical of this practice. TS's namespaces are a formalization of the module pattern. (The term "module pattern" is somewhat unfortunate now that ECMAScript 2015 formally supports modules in a manner different from what the module pattern prescribes. For this reason, TS uses the term "namespace" for its formalization of the module pattern.)

This section and the namespace section below will show how TS emits consistent, idiomatic JS when emitting ECMAScript 3 or 5 compliant code for classes and namespaces. The goal of TS's translation is to emit exactly what a programmer would type when implementing a class or namespace unaided by a tool. This section will also describe how TS infers a type for each class declaration. We'll start with a simple BankAccount class.
...
This class generates the following JS code.
...
This TS class declaration creates a variable named 'BankAccount' whose value is the constructor function for 'BankAccount' instances. This declaration also creates an instance type of the same name. If we were to write this type as an interface it would look like the following.
...
The function signature is prefixed with the keyword 'new' indicating that the 'BankAccount' function must be called as a constructor. It is possible for a function's type to have both call and constructor signatures. For example, the type of the built-in JS Date object includes both kinds of signatures.
...
This version of the 'BankAccount' class requires us to introduce a constructor parameter and then assign it to the 'balance' field. To simplify this common case, TS accepts the following shorthand syntax.
...
TS classes also support inheritance, as in the following example.* *
...
In this example, the class 'CheckingAccount' *derives* from class 'BankAccount'. The constructor for 'CheckingAccount' calls the constructor for class 'BankAccount' using the 'super' keyword. In the emitted JS code, the prototype of 'CheckingAccount' will chain to the prototype of 'BankAccount'. 

TS classes may also specify static members. Static class members become properties of the class constructor. 

Section [8](#8) provides additional information about classes.

## <a name="1.7"/>1.7 Enum Types

TS enables programmers to summarize a set of numeric constants as an *enum type*. The example below creates an enum type to represent operators in a calculator application.
...
When enums are declared with the `const` modifier, TS will emit for an enum member a JS constant corresponding to that member's assigned value (annotated with a comment). This improves performance on many JS engines.
...
JS implementations can use these explicit constants to generate efficient code for this switch statement, for example by building a jump table indexed by case value.

## <a name="1.8"/>1.8 Overloading on String Parameters

An important goal of TS is to provide accurate and straightforward types for existing JS programming patterns. To that end, TS includes generic types, discussed in the next section, and *overloading on string parameters*, the topic of this section.
...
Like overloading on string parameters, *generic types* make it easier for TS to accurately capture the behavior of JS libraries. Because they enable type information to flow from client code, through library code, and back into client code, generic types may do more than any other TS feature to support detailed API descriptions.

To illustrate this, let's take a look at part of the TS interface for the built-in JS array type. You can find this interface in the 'lib.d.ts' file that accompanies a TS distribution.
...
The TS compiler can often infer generic method parameters, making it unnecessary for the programmer to explicitly provide them. In the following example, the compiler infers that parameter 'U' of the map method has type 'string', because the function passed to map returns a string.
...
Classes and interfaces support large-scale JS development by providing a mechanism for describing how to use a software component that can be separated from that component's implementation. TS enforces *encapsulation* of implementation in classes at design time (by restricting use of private and protected members), but cannot enforce encapsulation at runtime because all object properties are accessible at runtime. Future versions of JS may provide *private names* which would enable runtime enforcement of private and protected members.

In JS, a very common way to enforce encapsulation at runtime is to use the module pattern: encapsulate private fields and methods using closure variables. The module pattern is a natural way to provide organizational structure and dynamic loading options by drawing a boundary around a software component. The module pattern can also provide the ability to introduce namespaces, avoiding use of the global namespace for most software components. 

The following example illustrates the JS module pattern.
...
TS namespaces provide a mechanism for succinctly expressing the module pattern. In TS, programmers can combine the module pattern with the class pattern by nesting namespaces and classes within an outer namespace.
...
TS also supports ECMAScript 2015 modules, which are files that contain top-level *export* and *import* directives. For this type of module TS can emit both ECMAScript 2015 compliant code and down-level ECMAScript 3 or 5 compliant code for a variety of module loading systems, including CommonJS, Asynchronous Module Definition (AMD), and Universal Module Definition (UMD).

<br/>

# <a name="2"/>2 Basic Concepts

The remainder of this document is the formal specification of the TS programming language and is intended to be read as an adjunct to the [ECMAScript 2015 Language Specification](http://www.ecma-international.org/ecma-262/6.0/) (specifically, the ECMA-262 Standard, 6th Edition). This document describes the syntactic grammar added by TS along with the compile-time processing and type checking performed by TS, but it only minimally discusses the run-time behavior of programs since that is covered by the ECMAScript specification.

## <a name="2.1"/>2.1 Grammar Conventions

The syntactic grammar added by TS language is specified throughout this document using the existing conventions and production names of the ECMAScript grammar. In places where TS augments an existing grammar production it is so noted. For example:
...
A core purpose of TS is to track the named entities in a program and validate that they are used according to their designated meaning. Names in TS can be written in several ways, depending on context. Specifically, a name can be written as
...
ECMAScript 2015 permits object literals and classes to declare members with computed property names. A computed property name specifies an expression that computes the actual property name at run-time. Because the final property name isn't known at compile-time, TS can only perform limited checks for entities declared with computed property names. However, a subset of computed property names known as ***well-known symbols*** can be used anywhere a *PropertyName* is expected, including property names within types. A computed property name is a well-known symbol if it is of the form
...
*TODO: Update to reflect treatment of [computed property names with literal expressions](https://github.com/Microsoft/TS/pull/5535)*.
...
*TODO: [Include specific rules for alias resolution](https://github.com/Microsoft/TS/issues/3158)*.
...
TS adds optional static types to JS. Types are used to place static constraints on program entities such as functions, variables, and properties so that compilers and development tools can offer better verification and assistance during software development. TS's *static* compile-time type system closely models the *dynamic* run-time type system of JS, allowing programmers to accurately express the type relationships that are expected to exist when their programs run and have those assumptions pre-validated by TS. TS's type analysis occurs entirely at compile-time and adds no run-time overhead to program execution.

All types in TS are subtypes of a single top type called the Any type. The `any` keyword references this type. The Any type is the one type that can represent *any* JS value with no constraints. All other types are categorized as ***primitive types***, ***object types***, ***union types***, ***intersection types***, or ***type parameters***. These types introduce various static constraints on their values.
...
The Any type is used to represent any JS value. A value of the Any type supports the same operations as a value in JS and minimal static type checking is performed for operations on Any values. Specifically, properties of any name can be accessed through an Any value and Any values can be called as functions or constructors with any argument list.

The `any` keyword references the Any type. In general, in places where a type is not explicitly provided and TS cannot infer one, the Any type is assumed.
...
The Number primitive type corresponds to the similarly named JS primitive type and represents double-precision 64-bit format IEEE 754 floating point values.
...
The Boolean primitive type corresponds to the similarly named JS primitive type and represents logical values that are either true or false.
...
The String primitive type corresponds to the similarly named JS primitive type and represents sequences of characters stored as Unicode UTF-16 code units.
...
The Symbol primitive type corresponds to the similarly named JS primitive type and represents unique tokens that may be used as keys for object properties.
...
The Null type corresponds to the similarly named JS primitive type and is the type of the `null` literal.
...
The Undefined type corresponds to the similarly named JS primitive type and is the type of the `undefined` literal.
...
*TODO: Update to reflect [expanded support for string literal types](https://github.com/Microsoft/TS/pull/5185)*.
...
Class and interface type references, array types, tuple types, function types, and constructor types are all classified as object types. Multiple constructs in the TS language create object types, including:
...
***Array types*** represent JS arrays with a common element type. Array types are named type references created from the generic interface type 'Array' in the global namespace with the array element type as a type argument. Array type literals (section [3.8.4](#3.8.4)) provide a shorthand notation for creating such references.
...
***Tuple types*** represent JS arrays with individually tracked element types. Tuple types are written using tuple type literals (section [3.8.5](#3.8.5)). A tuple type combines a set of numerically named properties with the members of an array type. Specifically, a tuple type
...
TS has a structural type system, and therefore an instantiation of a generic type is indistinguishable from an equivalent manually written expansion. For example, given the declaration
...
*TODO: Update to reflect [binding parameter cannot be optional in implementation signature](https://github.com/Microsoft/TS/issues/2797)*.

*TODO: Update to reflect [required parameters support initializers](https://github.com/Microsoft/TS/pull/4022)*.
...
Types in TS have identity, subtype, supertype, and assignment compatibility relationships as defined in the following sections.
...
*TODO: Update to reflect [improved union and intersection type inference](https://github.com/Microsoft/TS/pull/5738)*.
...
In several situations TS infers types from context, alleviating the need for the programmer to explicitly specify types that appear obvious. For example
...
This chapter describes the manner in which TS provides type inference and type checking for JS expressions. TS's type analysis occurs entirely at compile-time and adds no run-time overhead to expression evaluation.

TS's typing rules define a type for every expression construct. For example, the type of the literal 123 is the Number primitive type, and the type of the object literal { a: 10, b: "hello" } is { a: number; b: string; }. The sections in this chapter describe these rules in detail.

In addition to type inference and type checking, TS augments JS expressions with the following constructs:
...
Unless otherwise noted in the sections that follow, TS expressions and the JS expressions generated from them are identical.
...
*TODO: Document spreading an [iterator](https://github.com/Microsoft/TS/pull/2498) into an array literal*.
...
*TODO: [Template literals](https://github.com/Microsoft/TS/pull/960)*.
...
The JS code generated for a super call is specified in section [8.7.2](#8.7.2).
...
The JS code generated for a super property access is specified in section [8.7.2](#8.7.2).

*TODO: Update section to include [bracket notation in super property access](https://github.com/Microsoft/TS/issues/3970)*.

## <a name="4.10"/>4.10 Function Expressions

Function expressions are extended from JS to optionally include parameter and return type annotations.
...
Arrow functions are extended from JS to optionally include parameter and return type annotations.
...
The TS compiler applies this type of transformation to rewrite arrow function expressions into standard function expressions.
...
*TODO: Document [class expressions](https://github.com/Microsoft/TS/issues/497)*.
...
*TODO: Indexing with [symbols](https://github.com/Microsoft/TS/pull/1978)*.
...
Function calls are extended from JS to support optional type arguments.
...
If *func* has apparent call signatures (section [3.11.1](#3.11.1)) the call is a ***typed function call***. TS employs ***overload resolution*** in typed function calls in order to support functions with multiple call signatures. Furthermore, TS may perform ***type argument inference*** to automatically determine type arguments in generic function calls.

### <a name="4.15.1"/>4.15.1 Overload Resolution

The purpose of overload resolution in a function call is to ensure that at least one signature is applicable, to provide contextual types for the arguments, and to determine the result type of the function call, which could differ between the multiple applicable signatures. Overload resolution has no impact on the run-time behavior of a function call. Since JS doesn't support function overloading, all that matters at run-time is the name of the function.

*TODO: Describe use of [wildcard function types](https://github.com/Microsoft/TS/issues/3970) in overload resolution*.
...
*TODO: [Spread operator in function calls](https://github.com/Microsoft/TS/pull/1931) and spreading an [iterator](https://github.com/Microsoft/TS/pull/2498) into a function call*.
...
*TODO: Update [type argument inference and overload resolution rules](https://github.com/Microsoft/TS/issues/1186)*.
...
TS extends the JS expression grammar with the ability to assert a type for an expression:

&emsp;&emsp;*UnaryExpression:*  *( Modified )*  
&emsp;&emsp;&emsp;…  
&emsp;&emsp;&emsp;`<`&emsp;*Type*&emsp;`>`&emsp;*UnaryExpression*

A type assertion expression consists of a type enclosed in `<` and `>` followed by a unary expression. Type assertion expressions are purely a compile-time construct. Type assertions are *not* checked at run-time and have no impact on the emitted JS (and therefore no run-time cost). The type and the enclosing `<` and `>` are simply removed from the generated code.
...
*TODO: Document [as operator](https://github.com/Microsoft/TS/pull/3564)*.

## <a name="4.17"/>4.17 JSX Expressions

*TODO: Document [JSX expressions](https://github.com/Microsoft/TS/issues/3203)*.
...
*TODO: Document the [exponentation operator](https://github.com/Microsoft/TS/issues/4812)*.
...
*TODO: [Update to specify behavior when assignment element E is a rest element](https://github.com/Microsoft/TS/issues/2713)*.
...
When the output target is ECMAScript 2015 or higher, destructuring variable assignments remain unchanged in the emitted JS code. When the output target is ECMAScript 3 or 5, destructuring variable assignments are rewritten to series of simple assignments. For example, the destructuring assignment
...
*TODO: Document [user defined type guard functions](https://github.com/Microsoft/TS/issues/1007)*.
...
This chapter describes the static type checking TS provides for JS statements. TS itself does not introduce any new statement constructs, but it does extend the grammar for local declarations to include interface, type alias, and enum declarations.
...
*TODO: Document destructuring an [iterator](https://github.com/Microsoft/TS/pull/2498) into an array*.
...
*TODO: Update rules to reflect [improved checking of destructuring with literal initializers](https://github.com/Microsoft/TS/pull/4598)*.

When the output target is ECMAScript 2015 or higher, except for removing the optional type annotation, destructuring variable declarations remain unchanged in the emitted JS code.
...
The '_a' and '_b' temporary variables exist to ensure the assigned expression is evaluated only once, and the expression 'void 0' simply denotes the JS value 'undefined'.
...
*TODO: Document scoping and types of [let and const declarations](https://github.com/Microsoft/TS/pull/904)*.
...
*TODO: Document [for-of statements](https://github.com/Microsoft/TS/issues/7)*.
...
Use of the 'with' statement in TS is an error, as is the case in ECMAScript 5's strict mode. Furthermore, within the body of a 'with' statement, TS considers every identifier occurring in an expression (section [4.3](#4.3)) to be of the Any type regardless of its declared type. Because the 'with' statement puts a statically unknown set of identifiers in scope in front of those that are statically known, it is not possible to meaningfully assign a static type to any identifier.
...
TS extends JS functions to include type parameters, parameter and return type annotations, overloads, default parameter values, and rest parameters.
...
Function overloads are purely a compile-time construct. They have no impact on the emitted JS and thus no run-time cost.
...
When the output target is ECMAScript 3 or 5, for each parameter with an initializer, a statement that substitutes the default value for an omitted argument is included in the generated JS, as described in section [6.6](#6.6). The example
...
generates JS that is equivalent to
...
A function declaration generates JS code that is equivalent to:
...
*TODO: Document [generator functions](https://github.com/Microsoft/TS/issues/2873)*.

## <a name="6.8"/>6.8 Asynchronous Functions

*TODO: Document [asynchronous functions](https://github.com/Microsoft/TS/issues/1664)*.

## <a name="6.9"/>6.9 Type Guard Functions

*TODO: Document [type guard functions](https://github.com/Microsoft/TS/issues/1007), including [this type predicates](https://github.com/Microsoft/TS/pull/5906)*.
...
Because TS has a structural type system, an interface type with a particular set of members is considered identical to, and can be substituted for, another interface type or object type literal with an identical set of members (see section [3.11.2](#3.11.2)).
...
*TODO: Document [class and interface declaration merging](https://github.com/Microsoft/TS/pull/3333)*.
...
TS does not provide a direct mechanism for dynamically testing whether an object implements a particular interface. Instead, TS code can use the JS technique of checking whether an appropriate set of members are present on the object. For example, given the declarations in section [7.1](#7.1), the following is a dynamic check for the 'MoverShaker' interface:
...
TS extends JS classes to include type parameters, implements clauses, accessibility modifiers, member variable declarations, and parameter property declarations in constructors.

*TODO: Document [abstract classes](https://github.com/Microsoft/TS/issues/3578)*.
...
*TODO: Update this section to reflect [expressions in class extends clauses](https://github.com/Microsoft/TS/pull/3516)*.
...
Private and protected accessibility is enforced only at compile-time and serves as no more than an *indication of intent*. Since JS provides no mechanism to create private and protected properties on an object, it is not possible to enforce the private and protected modifiers in dynamic code at run-time. For example, private and protected accessibility can be defeated by changing an object's static type to Any and accessing the member dynamically.
...
Note that TS doesn't require or verify that derived constructor functions are subtypes of base constructor functions. In other words, changing the declaration of 'B' to
...
Get and set accessors are emitted as calls to 'Object.defineProperty' in the generated JS, as described in section [8.7.1](#8.7.1).
...
*TODO: Document [decorators](https://github.com/Microsoft/TS/issues/2249)*.
...
A class with no `extends` clause generates JS equivalent to the following:
...
A class with an `extends` clause generates JS equivalent to the following:
...
In addition, the '__extends' function below is emitted at the beginning of the JS source file. It copies all properties from the base constructor function object to the derived constructor function object (in order to inherit static members), and appropriately establishes the 'prototype' property of the derived constructor function object.
...
A super property access in the constructor, an instance member function, or an instance member accessor generates JS equivalent to

```TS
_super.prototype.<PropertyName>
```

where *PropertyName* is the name of the referenced base class property. When the super property access appears in a function call, the generated JS is equivalent to
...
A super property access in a static member function or a static member accessor generates JS equivalent to
...
Enum members are either ***constant members*** or ***computed members***. Constant members have known constant values that are substituted in place of references to the members in the generated JS code. Computed members have values that are computed at run-time and not known at compile-time. No substitution is performed for references to computed members.
...
Unlike regular enum declarations, constant enum declarations are completely erased in the emitted JS code. For this reason, it is an error to reference a constant enum object in any other context than a property access that selects one of the enum's members. For example:
...
The entire const enum declaration is erased in the emitted JS code. Thus, the only permitted references to the enum object are those that are replaced with an enum member value.

## <a name="9.5"/>9.5 Code Generation

An enum declaration generates JS equivalent to the following:
...
For example, the 'Color' enum example from section [9.1](#9.1) generates the following JS:
...
Namespaces are declared using the `namespace` keyword, but for backward compatibility of earlier versions of TS a `module` keyword can also be used.
...
*TODO: Clarify rules for [alias resolution](https://github.com/Microsoft/TS/issues/3158)*.
...
A namespace generates JS code that is equivalent to the following:
...
TS implements support for ECMAScript 2015 modules and supports down-level code generation targeting CommonJS, AMD, and other module systems.

## <a name="11.1"/>11.1 Programs and Source Files

A TS ***program*** consists of one or more source files.
...
Declaration source files are a strict subset of implementation source files and are used to declare the static type information associated with existing JS code in an adjunct manner. They are entirely optional but enable TS and tools to provide better verification and assistance when integrating existing JS code and libraries in a TS application.

When a TS program is compiled, all of the program's source files are processed together. Statements and declarations in different source files can depend on each other, possibly in a circular fashion. By default, a JS output file is generated for each implementation source file in a compilation, but no output is generated from declaration source files.

### <a name="11.1.1"/>11.1.1 Source Files Dependencies

The TS compiler automatically determines a source file's dependencies and includes those dependencies in the program being compiled. The determination is made from "reference comments" and module import declarations as follows:
...
The initialization order of the scripts that make up the global namespace ultimately depends on the order in which the generated JS files are loaded at run-time (which, for example, may be controlled by &lt;script/> tags that reference the generated JS files).
...
Initialization order of modules is determined by the module loader being used and is not specified by the TS language. However, it is generally the case that non-circularly dependent modules are automatically loaded and initialized in the correct order.
...
TS supports multiple patterns of JS code generation for modules:
...
For purposes of resolving module references, TS associates a file path with every module. The file path is simply the path of the module's source file without the file extension. For example, a module contained in the source file 'C:\src\lib\io.ts' has the file path 'C:/src/lib/io' and a module contained in the source file 'C:\src\ui\editor.d.ts' has the file path 'C:/src/ui/editor'.
...
Import require declarations exist for backward compatibility with earlier versions of TS.
...
Export assignments exist for backward compatibility with earlier versions of TS. An export assignment designates a module member as the entity to be exported in place of the module itself.
...
The [CommonJS Modules](http://www.commonjs.org/specs/modules/1.0/) definition specifies a methodology for writing JS modules with implied privacy, the ability to import other modules, and the ability to explicitly export members. A CommonJS compliant system provides a 'require' function that can be used to synchronously load other modules to obtain their singleton module instance, as well as an 'exports' variable to which a module can add properties to define its external API.

The 'main' and 'log' example from section [11.3](#11.3) above generates the following JS code when compiled for the CommonJS Modules pattern:
...
The 'game' module references the imported 'geometry' module in an expression (through its alias 'g') and a 'require' call is therefore included in the emitted JS:
...
the emitted JS would have no dependency on the 'geometry' module and would simply be
...
The "main" and "log" example from above generates the following JS code when compiled for the AMD pattern.
...
Ambient declarations are used to provide static typing over existing JS code. Ambient declarations differ from regular declarations in that no JS code is emitted for them. Instead of introducing new variables, functions, classes, enums, or namespaces, ambient declarations provide type information for entities that exist "ambiently" and are included in a program by external means, for example by referencing a JS library in a &lt;script/> tag.
