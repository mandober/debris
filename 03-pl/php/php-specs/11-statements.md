...
- [The `echo` Statement](#the-echo-statement)
- [The `unset` Statement](#the-unset-statement)
...
The `return` statement from within a function terminates the execution of that function normally, and depending on its definition (i.e. `&` prefixed function's name means byRef return) it returns the value of *expression* to the the caller by value or byRef.

If *expression* is omitted the value `NULL` is used. If execution flows into the closing brace of a function, `return NULL` is implied.
